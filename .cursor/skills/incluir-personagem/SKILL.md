...
2. Medir ímãs com **Project → Tools → Ímãs das Peças**: arraste cada bolinha até a esfera de metal. No tronco são 5 (pescoço, ombros, quadris).
...
4. Conferir `data/parts/{id}_*.tres` e `{id}_character.tres`. A loja pega o set sozinha (ou via `ACTIVE_SET_IDS` se houver filtro de teste).
